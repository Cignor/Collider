...
class ImGuiNodeEditorComponent : public juce::Component, private juce::OpenGLRenderer
{
public:
    ...
    // ADD: Callback for checking for updates
    std::function<void()> onCheckForUpdates;

    // ADD: Callback for rendering update dialog (ImGui)
    std::function<void()> onRenderUpdateDialog;
    ...
    void requestFontAtlasRebuild() { fontAtlasNeedsRebuild.store(true, std::memory_order_relaxed); }
    ...
    void handleMidiPlayerAutoConnect(
        MIDIPlayerModuleProcessor* midiPlayer,
        juce::uint32               midiPlayerLid);
    void handleMidiPlayerAutoConnectVCO(
        MIDIPlayerModuleProcessor* midiPlayer,
        juce::uint32               midiPlayerLid);
    void handleMidiPlayerAutoConnectHybrid(
        MIDIPlayerModuleProcessor* midiPlayer,
        juce::uint32               midiPlayerLid);
    ...
    static int encodePinId(const PinID& pinId)
    {
        ...
        juce::uint32 encoded = PIN_ID_FLAG | (pinId.isInput ? IS_INPUT_FLAG : 0) |
    ...
    static PinID decodePinId(int id)
    {
        ...
        if ((uid & PIN_ID_FLAG) == 0)
        {
            ...
            juce::Logger::writeToLog(
                "[ERROR] decodePinId called with non-pin ID: " + juce::String((int)id));
            pinId.logicalId = 0;
            pinId.channel = 0;
            pinId.isInput = false;
            pinId.isMod = false;
    ...
    struct LinkKeyHash
    {
        size_t operator()(const LinkKey& k) const noexcept
        {
            return ((size_t)k.srcAttr << 32) ^ (size_t)k.dstAttr;
        }
    };
    struct LinkKeyEq
    {
        bool operator()(const LinkKey& a, const LinkKey& b) const noexcept
        {
            return a.srcAttr == b.srcAttr && a.dstAttr == b.dstAttr;
        }
    };
    ...
    static constexpr juce::uint32 kOutputHighlightId =
        0xFFFFFFFFu;           // sentinel for main output node highlight
    int lastHoveredLinkId{-1}; // cache inside-editor hovered link id for post-editor use
    int m_currentlyProbedLinkId{
        -1}; // caches the last link sent to the probe to avoid redundant graph rebuilds
    ...
    struct ShortcutActionIds
    {
        ...
        static inline const juce::Identifier editDisconnectSelection{
            "actions.edit.disconnectSelection"};
        static inline const juce::Identifier editDuplicate{"actions.edit.duplicate"};
        static inline const juce::Identifier editDuplicateWithRouting{
            "actions.edit.duplicateWithRouting"};
        ...
        static inline const juce::Identifier viewToggleShortcutsWindow{
            "actions.view.toggleShortcutsWindow"};
        ...
        static inline const juce::Identifier graphConnectSelectedToTrackMixer{
            "actions.graph.connectSelectedToTrackMixer"};
        static inline const juce::Identifier graphConnectSelectedToRecorder{
            "actions.graph.connectSelectedToRecorder"};
    ...
    std::map<std::pair<juce::uint32, int>, ChannelHistory>
          inspectorHistory; // key: (logicalId, channel)
    ...
    std::unordered_map<int, std::unique_ptr<juce::OpenGLTexture>>& getVisionModuleTextures()
    {
        return visionModuleTextures;
    }
    ...
    void muteNodeSilent(
        juce::uint32 logicalId); // Store mute state without modifying graph (for loading presets)
    ...
    // Helper functions
    void insertNodeOnLink(
        const juce::String& nodeType,
        const LinkInfo&     linkInfo,
        const ImVec2&       position);
    void insertNodeOnLinkStereo(
        const juce::String& nodeType,
    ...
    std::vector<AudioPin> getPinsOfType(
