...
SplashScreenComponent::SplashScreenComponent()
{
    ...
    juce::Logger::writeToLog(
        "[Splash] Component initialized, size: " + juce::String(getWidth()) + "x" +
...
void SplashScreenComponent::loadSplashImage()
{
    ...
    juce::String splashFilename = "icons/splash.png";
#if AUDIO_ONLY_BUILD
    splashFilename = "icons/splash-audio.png";
#endif

    juce::File splashFile = exeDir.getChildFile(splashFilename);
    ...
    juce::File fallbackFile = projectRoot.getChildFile(splashFilename);
    ...
    if (fallbackFile.existsAsFile())
    {
        ...
        if (loadedImage.isValid())
        {
            splashImage = loadedImage;
            juce::Logger::writeToLog(
                "[Splash] Loaded image from project root: " + fallbackFile.getFullPathName());
            return;
        }
    }

    juce::Logger::writeToLog(
        "[Splash] Warning: Could not load splash screen image from " +
        splashFile.getFullPathName() + " or " + fallbackFile.getFullPathName());
}
...
void SplashScreenComponent::paint(juce::Graphics& g)
{
    ...
    auto      bounds = juce::Rectangle<int>(
        leftMargin,
        topPadding,
        getWidth() - leftMargin - padding,
        getHeight() - topPadding - padding);
    ...
    g.drawFittedText(
        VersionInfo::APPLICATION_NAME, titleBounds, juce::Justification::centredLeft, 1);
    ...
    auto hintBounds =
        juce::Rectangle<int>(padding, getHeight() - bottomPadding, getWidth() - padding * 2, 18);
    g.drawFittedText(
        "Press any key or click to continue", hintBounds, juce::Justification::centredBottom, 1);

    // Help hint below dismiss hint - smaller and more subtle
    g.setColour(juce::Colour(0xff666666));
    g.setFont(juce::Font(11.0f, juce::Font::plain));
    auto helpBounds = juce::Rectangle<int>(
        padding, getHeight() - bottomPadding + hintSpacing + 18, getWidth() - padding * 2, 16);
    g.drawFittedText("Press F1 for Help", helpBounds, juce::Justification::centredBottom, 1);
}
...
void SplashScreenComponent::mouseWheelMove(
