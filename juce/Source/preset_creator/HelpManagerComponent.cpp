...
void HelpManagerComponent::parseMarkdown(const juce::String& content, std::vector<MarkdownSection>& sections)
{
    ...
    for (int i = 0; i < lines.size(); ++i)
    {
        ...
        // Skip horizontal rules (---) - they're just separators, not content
        if (line == "---")
            continue;
...
void HelpManagerComponent::renderMarkdownSection(const MarkdownSection& section, const juce::String& searchTerm, bool parentMatches, bool forceExpand)
{
    ...
    {
        // Level 3+ (####) - enhanced styled header with distinct colors and larger font
        ...
        ImGui::Spacing();
        ImGui::Spacing(); // Extra spacing before Level 3 headers
        ImGui::PushID(section.anchor.toRawUTF8()); // Unique ID for each subsection
        
        // Determine color based on section type for better visual rhythm
        ImVec4 subHeaderColor;
        juce::String titleUpper = section.title.toUpperCase();
        
        if (titleUpper.contains("INPUTS"))
        {
            // Inputs - use CV pin color (typically blue/cyan)
            ImU32 pinColor = ThemeManager::getInstance().getPinColor(PinDataType::CV);
            subHeaderColor = ImGui::ColorConvertU32ToFloat4(pinColor);
            subHeaderColor.w = 1.0f; // Full opacity
        }
        else if (titleUpper.contains("OUTPUTS"))
        {
            // Outputs - use Audio pin color (typically green)
            ImU32 pinColor = ThemeManager::getInstance().getPinColor(PinDataType::Audio);
            subHeaderColor = ImGui::ColorConvertU32ToFloat4(pinColor);
            subHeaderColor.w = 1.0f;
        }
        else if (titleUpper.contains("PARAMETERS"))
        {
            // Parameters - use a warm accent color (orange/yellow)
            ImVec4 accent = ImGui::GetStyleColorVec4(ImGuiCol_HeaderHovered);
            subHeaderColor = ImVec4(accent.x * 1.2f, accent.y * 0.9f, accent.z * 0.7f, 1.0f);
        }
        else if (titleUpper.contains("HOW TO USE") || titleUpper.contains("HOW TO"))
        {
            // How to Use - use a bright informative color (cyan/teal)
            ImVec4 accent = ImGui::GetStyleColorVec4(ImGuiCol_HeaderHovered);
            subHeaderColor = ImVec4(accent.x * 0.7f, accent.y * 1.1f, accent.z * 1.2f, 1.0f);
        }
        else
        {
            // Default - bright accent color
            subHeaderColor = ImGui::GetStyleColorVec4(ImGuiCol_HeaderHovered);
            subHeaderColor.w = 1.0f;
            subHeaderColor.x = std::min(1.0f, subHeaderColor.x * 1.15f);
            subHeaderColor.y = std::min(1.0f, subHeaderColor.y * 1.15f);
            subHeaderColor.z = std::min(1.0f, subHeaderColor.z * 1.15f);
        }
        
        // Apply bold, larger font with distinct color
        ImGui::PushStyleColor(ImGuiCol_Text, subHeaderColor);
        ImGui::SetWindowFontScale(1.4f); // Larger font for better visibility
        ImGui::TextUnformatted(section.title.toRawUTF8());
        ImGui::SetWindowFontScale(1.0f);
        ImGui::PopStyleColor();
        ImGui::PopID();
...
void HelpManagerComponent::buildNavigationList(const std::vector<MarkdownSection>& sections, std::vector<NavigationItem>& navItems, int level)
{
    ...
    {
        // Skip Level 3 sections (Inputs, Outputs, Parameters, How to Use) from navigation
        // They should only appear in the content window, not the sidebar
        if (section.level == 3)
        {
            // Still recurse into children in case there are deeper levels
            if (!section.children.empty())
            {
                buildNavigationList(section.children, navItems, level);
            }
            continue;
        }
        
        // Add sections to navigation, but indent based on hierarchy
        NavigationItem item;
