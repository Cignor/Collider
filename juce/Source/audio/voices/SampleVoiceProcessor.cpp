...
void SampleVoiceProcessor::prepareToPlay(double rate, int samplesPerBlock)
{
    ...
    juce::Logger::writeToLog(
        "[SampleVoice] prepareToPlay sr=" + juce::String(rate) +
...
void SampleVoiceProcessor::renderBlock(juce::AudioBuffer<float>& buffer, juce::MidiBuffer& midi)
{
    ...
    if (sourceSample == nullptr || sourceSample->stereo.getNumSamples() < 2 ||
    ...
    const float apTime = apvts.getRawParameterValue("timeStretchRatio")
                             ? apvts.getRawParameterValue("timeStretchRatio")->load()
                             : 1.0f;
    const float apPitch = apvts.getRawParameterValue("pitchSemitones")
                              ? apvts.getRawParameterValue("pitchSemitones")->load()
                              : 0.0f;
    ...
    if (!smoothingEnabled)
    {
        smoothedTimeRatio = effectiveTime;
        smoothedPitchSemis = effectivePitchSemis;
        if (resetOnChangeWhenNoSmoothing)
            timePitch.reset();
    }
    else
    {
    ...
    if (engine.load(std::memory_order_relaxed) == Engine::Naive)
    {
        ...
        const double effectiveEndSample = (endSamplePos < 0.0 || endSamplePos >= sourceLength)
        ...
        for (int i = 0; i < numDestSamples; ++i)
        {
            ...
            // DEFENSIVE: Clamp base to valid range (prevents crash on sample switch)
            base = juce::jlimit(0, sourceLength - 1, base);
    ...
    for (int i = 0; i < numDestSamples; ++i)
    {
        ...
        // DEFENSIVE: Clamp pos to valid range (prevents crash on sample switch)
        pos = juce::jlimit(0, sourceLength - 1, pos);
    ...
    if (produced > 0)
    {
        ...
        {
            buffer.clear(0, produced, numDestSamples - produced);
            if (destR)
                buffer.clear(1, produced, numDestSamples - produced);
        }
    }
}
