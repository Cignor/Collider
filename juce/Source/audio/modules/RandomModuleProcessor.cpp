...
std::optional<RhythmInfo> RandomModuleProcessor::getRhythmInfo() const
{
    RhythmInfo info;
    
    // Build display name with logical ID
    info.displayName = "Random #" + juce::String(getLogicalId());
    info.sourceType = "random";
    ...
    // Random module is always active when running
    info.isActive = true;
    ...
    if (syncEnabled && hasTransport && transport.isPlaying)
    {
        ...
        // (One random value generation = one "beat" at the division rate)
