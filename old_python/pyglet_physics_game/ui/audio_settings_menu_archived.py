...
class AudioSettingsMenu:
	...
	def draw(self) -> None:
		...
		for i, name in enumerate(self._folders):
			...
			color = (255, 255, 255, 255) if is_selected else (220, 220, 220, 255)
			text.Label(name, x=col1_x + 8, y=row_y, color=color).draw()
		...
		for vis_i, idx in enumerate(range(start, end)):
			...
			color = (0, 255, 255, 255) if is_hover else ((255, 255, 0, 255) if is_selected else (220, 220, 220, 255))
			text.Label(items[idx], x=col2_x + 8, y=row_y, color=color).draw()
